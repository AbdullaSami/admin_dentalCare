                    ...
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @foreach ($doctors as $doctor)
                                <tr>
                                    <td class="px-2 py-4">{{ $doctors->firstItem() + $loop->index }}</td>
                                    ...
                                    <td class="flex px-6 py-4">
                                        <a href="{{ route('registered-doctors.edit', $doctor->id) }}" class="pr-4 hover:text-blue-700 transition-transform duration-300 ease-in-out hover:scale-110">
                                        ...
                                        <form method="POST" action="{{ route('registered-doctors.destroy', $doctor->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button class="hover:text-red-700 transition-transform duration-300 ease-in-out hover:scale-110" type="submit">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
